...
def main():
    ...
    if important_changes:
        print("⚠️  Warning: You have uncommitted changes to source files:")
    ...
    try:
        ...
        run_command("git rm -rf . 2>/dev/null || true")
    ...
    for item in Path('.').iterdir():
        if item.name != '.git' and item.name != 'output':
            if item.is_dir():
                shutil.rmtree(item)
            else:
                item.unlink()
    
    # Copy all files from output/site to root
